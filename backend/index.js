...
app.post('/api/reportes/clientes-frecuentes', async (req, res) => {
  const { fecha_inicio, fecha_fin, min_reservas, min_gasto, antiguedad } = req.body;

  try {
    let query = `
      SELECT c.id, c.nombre, c.apellido, c.correo,
             COUNT(r.id) AS total_reservas,
             SUM(p.monto)::numeric(10,2) AS total_gastado,
    ...
    query += ` GROUP BY c.id, c.nombre, c.apellido, c.correo`;
    ...
    console.error('Error real en clientes frecuentes:', err);
...
app.post('/api/reportes/cancelaciones', async (req, res) => {
    ...
    try {
      let query = `
        SELECT r.id, c.nombre AS cliente, ca.nombre AS cancha,
               r.fecha_inicio, r.fecha_cancelacion, p.monto::numeric(10,2) AS monto_perdido
        FROM Reservas r
        JOIN Clientes c ON r.id_cliente = c.id
        JOIN Canchas ca ON r.id_cancha = ca.id
        LEFT JOIN Pagos p ON p.id_reserva = r.id
        WHERE r.estado = 'Cancelada'
          AND r.fecha_cancelacion BETWEEN $1 AND $2
      `;
      ...
      if (horas_anticipacion) {
        query += ` AND EXTRACT(EPOCH FROM (r.fecha_inicio - r.fecha_cancelacion))/3600 <= $${i++}`;
      ...
      console.error('Error al generar reporte de cancelaciones:', err);
